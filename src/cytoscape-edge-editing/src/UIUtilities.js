...
module.exports = function (params, cy) {
  ...
  var addBendPointCxtMenuId =
    "cy-edge-bend-editing-cxt-add-bend-point" + stageId;
  var removeBendPointCxtMenuId =
    "cy-edge-bend-editing-cxt-remove-bend-point" + stageId;
  var removeAllBendPointCtxMenuId =
    "cy-edge-bend-editing-cxt-remove-multiple-bend-point" + stageId;
  var addControlPointCxtMenuId =
    "cy-edge-control-editing-cxt-add-control-point" + stageId;
  var removeControlPointCxtMenuId =
    "cy-edge-control-editing-cxt-remove-control-point" + stageId;
  var removeAllControlPointCtxMenuId =
    "cy-edge-bend-editing-cxt-remove-multiple-control-point" + stageId;
  var eStyle,
    eRemove,
    eAdd,
    eZoom,
      ...
      */
      ...
      var anchorManager = {
        ...
        eMouseDown: function (event) {
          ...
          var weightStr =
            anchorPointUtilities.syntax[anchorManager.edgeType]["weight"];
          var distanceStr =
            anchorPointUtilities.syntax[anchorManager.edgeType]["distance"];

          var edge = anchorManager.edge;
          moveAnchorParam = {
            edge: edge,
            type: anchorManager.edgeType,
            weights: edge.data(weightStr)
              ? [].concat(edge.data(weightStr))
              : [],
            distances: edge.data(distanceStr)
              ? [].concat(edge.data(distanceStr))
              : [],
          };
          ...
          canvas
            .getStage()
            .on("contentTouchend contentMouseup", anchorManager.eMouseUp);
        ...
        eMouseUp: function (event) {
          ...
          canvas
            .getStage()
            .off("contentTouchend contentMouseup", anchorManager.eMouseUp);
        ...
        renderAnchorShapes: function (edge) {
          ...
          if (
            !edge.hasClass("edgebendediting-hasbendpoints") &&
        ...
        renderAnchorShape: function (anchorX, anchorY, length) {
          ...
          var renderedTopLeftPos = convertToRenderedPosition({
            x: topLeftX,
            y: topLeftY,
          });
          length *= cy.zoom();
      ...
      var cxtRemoveAnchorFcn = function (event) {
        ...
        if (
          anchorPointUtilities.edgeTypeNoneShouldntHappen(
            type,
        ...
        var param = {
          ...
          type: type,
          weights: [].concat(
            edge.data(anchorPointUtilities.syntax[type]["weight"])
          ),
          distances: [].concat(
            edge.data(anchorPointUtilities.syntax[type]["distance"])
          ),
        };
      ...
      var menuItems = [
        ...
        },
        {
          ...
          selector:
            opts.enableMultipleAnchorRemovalOption &&
      ...
      function options() {
        return (
          optCache || (optCache = $container.data("cyedgeediting").options)
        );
      }
      ...
      function renderEachEndPointShape(
        source,
        target,
        length,
        nextToSource,
        ...
        // convert to rendered parameters
        var renderedSourcePos = convertToRenderedPosition({
          x: sTopLeftX,
          y: sTopLeftY,
        });
        var renderedTargetPos = convertToRenderedPosition({
          x: tTopLeftX,
          y: tTopLeftY,
        });
        length = (length * cy.zoom()) / 2;

        var renderedNextToSource = convertToRenderedPosition({
          x: nextToSourceX,
          y: nextToSourceY,
        });
        var renderedNextToTarget = convertToRenderedPosition({
          x: nextToTargetX,
          y: nextToTargetY,
        });
        ...
        var distanceSource = Math.sqrt(
          Math.pow(renderedNextToSource.x - renderedSourcePos.x, 2) +
            Math.pow(renderedNextToSource.y - renderedSourcePos.y, 2)
        );
        var sourceEndPointX =
          renderedSourcePos.x +
          (distanceFromNode / distanceSource) *
            (renderedNextToSource.x - renderedSourcePos.x);
        var sourceEndPointY =
          renderedSourcePos.y +
          (distanceFromNode / distanceSource) *
            (renderedNextToSource.y - renderedSourcePos.y);

        var distanceTarget = Math.sqrt(
          Math.pow(renderedNextToTarget.x - renderedTargetPos.x, 2) +
            Math.pow(renderedNextToTarget.y - renderedTargetPos.y, 2)
        );
        var targetEndPointX =
          renderedTargetPos.x +
          (distanceFromNode / distanceTarget) *
            (renderedNextToTarget.x - renderedTargetPos.x);
        var targetEndPointY =
          renderedTargetPos.y +
          (distanceFromNode / distanceTarget) *
            (renderedNextToTarget.y - renderedTargetPos.y);
      ...
      function getAnchorShapesLength(edge) {
        ...
        if (parseFloat(edge.css("width")) <= 2.5) return 2.5 * factor;
      ...
      function getContainingShapeIndex(x, y, edge) {
        ...
        if (
          edge.data(anchorPointUtilities.syntax[type]["weight"]) == null ||
      ...
      function getContainingEndPoint(x, y, edge) {
        ...
        if (checkIfInsideShape(x, y, length, src.x, src.y)) return 0;
        else if (checkIfInsideShape(x, y, length, target.x, target.y)) return 1;
        else return -1;
      }
      ...
      function turnOffActiveBgColor() {
        ...
        if (cy.style()._private.coreStyle["active-bg-opacity"]) {
          lastActiveBgOpacity =
            cy.style()._private.coreStyle["active-bg-opacity"].value;
        } else {
      ...
      function moveAnchorPoints(positionDiff, edges) {
        edges.forEach(function (edge) {
          var previousAnchorsPosition =
            anchorPointUtilities.getAnchorsAsArray(edge);
          var nextAnchorPointsPosition = [];
          if (previousAnchorsPosition != undefined) {
            for (var i = 0; i < previousAnchorsPosition.length; i += 2) {
              nextAnchorPointsPosition.push({
                x: previousAnchorsPosition[i] + positionDiff.x,
                y: previousAnchorsPosition[i + 1] + positionDiff.y,
              });
            }
            var type = anchorPointUtilities.getEdgeType(edge);
            ...
            if (type === "bend") {
              params.bendPointPositionsSetterFunction(
                edge,
                nextAnchorPointsPosition
              );
            } else if (type === "control") {
              params.controlPointPositionsSetterFunction(
                edge,
                nextAnchorPointsPosition
              );
            }
      ...
      function moveAnchorOnDrag(edge, type, index, position) {
        ...
        var distances = edge.data(
          anchorPointUtilities.syntax[type]["distance"]
        );

        var relativeAnchorPosition =
          anchorPointUtilities.convertToRelativePosition(edge, position);
...

      {
        ...
        cy.on(
          ...
          (eSelect = function () {
            ...
            if (
              edge.target().connectedEdges().length == 0 ||
              edge.source().connectedEdges().length == 0
            ) {
              return;
            }
        ...
        cy.on(
          "tapstart",
          "edge",
          (eTapStartOnEdge = function (event) {
            var edge = this;
            ...
            if (endPoint == 0 || endPoint == 1) {
              ...
              detachedNode =
                endPoint == 0 ? movedEdge.source() : movedEdge.target();

              var disconnectedEnd = endPoint == 0 ? "source" : "target";
              var result = reconnectionUtilities.disconnectEdge(
                movedEdge,
                cy,
        ...
        cy.on(
          ...
          (eTapDrag = function (event) {
            ...
            if (
              movedEdge !== undefined &&
              anchorPointUtilities.isIgnoredEdge(edge)
            ) {
              return;
            }
            ...
            if (
              ...
              movedAnchorIndex = anchorPointUtilities.addAnchorPoint(
                edge,
                tapStartPos
              );
              movedEdge = edge;
          ...
          (eTapEnd = function (event) {
            ...
            if (edge !== undefined) {
              ...
              if (index != undefined) {
                ...
                if (
                  (anchor.x === preAnchorPoint.x &&
                    anchor.y === preAnchorPoint.y) ||
                  (anchor.x === preAnchorPoint.x &&
                    anchor.y === preAnchorPoint.y)
                ) {
                  nearToLine = true;
                  ...
                  var m1 =
                    (preAnchorPoint.y - posAnchorPoint.y) /
                    (preAnchorPoint.x - posAnchorPoint.x);
                  var m2 = -1 / m1;
                  ...
                  var currentIntersection =
                    anchorPointUtilities.getIntersection(
                      edge,
                      anchor,
                      srcTgtPointsAndTangents
                    );
                  var dist = Math.sqrt(
                    Math.pow(anchor.x - currentIntersection.x, 2) +
                  ...
                  if (
                    type === "bend" &&
                    dist < options().bendRemovalSensitivity
                  ) {
                    nearToLine = true;
                  }
                }
                ...
                dummyNode != undefined &&
                (movedEndPoint == 0 || movedEndPoint == 1)
              ) {
                ...
                if (nodeToAttach) {
                  var newSource =
                    movedEndPoint == 0 ? nodeToAttach : edge.source();
                  var newTarget =
                    movedEndPoint == 1 ? nodeToAttach : edge.target();
                ...
                edge = reconnectionUtilities.connectEdge(
                  edge,
                  detachedNode,
                  location
                );
                ...
                if (detachedNode.id() !== newNode.id()) {
                  ...
                  if (typeof handleReconnectEdge === "function") {
                    var reconnectedEdge = handleReconnectEdge(
                      newSource.id(),
                      newTarget.id(),
                      edge.data()
                    );
                    ...
                    var oldLoc =
                      movedEndPoint == 0
                        ...
                        : { target: detachedNode.id() };

                    if (
                      options().undoable &&
                      newNode.id() !== detachedNode.id()
                    ) {
                      var param = {
                        edge: edge,
                        location: loc,
                ...
                if (
                  isValid !== "valid" &&
                  typeof actOnUnsuccessfulReconnection === "function"
                ) {
            ...
            if (
              edge !== undefined &&
              moveAnchorParam !== undefined &&
              (edge.data(weightStr) ? edge.data(weightStr).toString() : null) !=
                moveAnchorParam.weights.toString()
            ) {
        ...
        cy.on("edgeediting.movestart", function (e, edges) {
          ...
          if (edges[0] != undefined) {
            edges.forEach(function (edge) {
              if (
                anchorPointUtilities.getAnchorsAsArray(edge) != undefined &&
                !firstAnchorPointFound
              ) {
                firstAnchor = {
                  x: anchorPointUtilities.getAnchorsAsArray(edge)[0],
                  y: anchorPointUtilities.getAnchorsAsArray(edge)[1],
                };
                moveanchorparam = {
                  firstTime: true,
                  firstAnchorPosition: {
        ...
        cy.on("edgeediting.moveend", function (e, edges) {
          if (moveanchorparam != undefined) {
            ...
            var movedFirstAnchor = {
              x: anchorPointUtilities.getAnchorsAsArray(
                edgeContainingFirstAnchor
              )[0],
              y: anchorPointUtilities.getAnchorsAsArray(
                edgeContainingFirstAnchor
              )[1],
            };
      ...
      function keyDown(e) {
        var shouldMove =
          typeof options().moveSelectedAnchorsOnKeyEvents === "function"
            ? options().moveSelectedAnchorsOnKeyEvents()
            : options().moveSelectedAnchorsOnKeyEvents;
        ...
        if (tn != "TEXTAREA" && tn != "INPUT") {
          ...
          if (
            cy.edges(":selected").length != cy.elements(":selected").length ||
            cy.edges(":selected").length != 1
          ) {
            return;
          }
          if (!anchorsMoving) {

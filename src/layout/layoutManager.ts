...
class LayoutManager {
  ...
  public runOnce() {
    ...
    const layout = this.cy.makeLayout({
      ...this.currentLayout,
      constraints: this.groups.flatMap(group => group.nodes.map(node => ({
        type: 'position',
        node: node.id(),
  ...
  public lockNodes(nodes: cytoscape.NodeCollection) {
  ...
  public unlockNodes(groupId: string) {
    const groupNode = this.cy.getElementById(groupId);
    this.cy.nodes().filter(node => node.parent() === groupNode).grabify()
